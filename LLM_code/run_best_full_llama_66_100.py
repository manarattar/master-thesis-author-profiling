"""
Ablation: replace "66-" label with "66-100" in both prompt and parser.
Hypothesis: the open-ended "66-" looks syntactically broken to the model;
"66-100" gives it a proper range and may increase recall for the oldest group.

Results are normalised back to "66-" before evaluation so gold labels match.
"""
...
from prompts import JOINT_PROMPTS, GENDER_LABELS, AGE_LABELS

# config

BEST_PROMPT = "demographic"
BEST_MODE   = "joint"
LABEL_VARIANT = "66-100"   # what the model sees
...
OUTPUT_CSV = "llm_final_results_llama_66_100.csv"
CHECKPOINT_CSV = "llm_final_results_llama_66_100_checkpoint.csv"
REQUEST_TIMEOUT = 60
...
def parse_age(text: str):
    m = re.search(r"Age:\s*(0-25|26-35|36-65|66-100)", text)
    if m:
        val = m.group(1)
        return "66-" if val == "66-100" else val   # normalise back to gold label
    return None
...
def print_metrics(gold, pred, labels, task):
    ...
    print(f"\n{'='*60}")
    print(f"{task.upper()} RESULTS — {MODEL} [66-100 variant]")
    print(f"{'='*60}")
...
def main():
    print(f"Running ABLATION: label variant='{LABEL_VARIANT}', prompt='{BEST_PROMPT}', mode='{BEST_MODE}'")
    ...
    # Replace "66-" with "66-100" everywhere in the prompt
    prompt_template = JOINT_PROMPTS[f"joint_{BEST_PROMPT}"].replace("66-", "66-100")
    ...
    if len(gender_df) > 0:
        save_confusion_matrix(
            ...
            f"Gender — llama ({BEST_PROMPT}, 66-100)",
            "llm_gender_cm_llama_66_100.png"
        )
    if len(age_df) > 0:
        save_confusion_matrix(
            age_df["age"], age_df["pred_age"], AGE_LABELS,
            f"Age — llama ({BEST_PROMPT}, 66-100)",
            "llm_age_cm_llama_66_100.png"
        )
