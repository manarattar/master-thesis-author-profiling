"""
Ablation: replace "66-" label with "66+" in both prompt and parser.
Hypothesis: "66+" is a conventional shorthand the model has seen more often
than the open-ended "66-" (no upper bound), which may look like a typo.
...
BEST_MODE   = "joint"
LABEL_VARIANT = "66+"   # what the model sees
...
OUTPUT_CSV = "llm_final_results_llama_66plus.csv"
CHECKPOINT_CSV = "llm_final_results_llama_66plus_checkpoint.csv"
REQUEST_TIMEOUT = 60
...
def parse_age(text: str):
    ...
    if m:
        val = m.group(1)
        return "66-" if val == "66+" else val   # normalise back to gold label
    return None
...
def print_metrics(gold, pred, labels, task):
    ...
    print(f"\n{'='*60}")
    print(f"{task.upper()} RESULTS — {MODEL} [66+ variant]")
    print(f"{'='*60}")
...
def main():
    ...
    # Replace "66-" with "66+" everywhere in the prompt
    prompt_template = JOINT_PROMPTS[f"joint_{BEST_PROMPT}"].replace("66-", "66+")
    ...
    if len(gender_df) > 0:
        save_confusion_matrix(
            ...
            f"Gender — llama ({BEST_PROMPT}, 66+)",
            "llm_gender_cm_llama_66plus.png"
        )
    if len(age_df) > 0:
        save_confusion_matrix(
            age_df["age"], age_df["pred_age"], AGE_LABELS,
            f"Age — llama ({BEST_PROMPT}, 66+)",
            "llm_age_cm_llama_66plus.png"
        )
