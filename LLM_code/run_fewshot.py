"""
Few-shot LLM experiment on LiLaH EN.

Loads 8 annotated examples from data/fewshot_examples.csv, injects them
into JOINT_FEWSHOT, excludes those rows from evaluation, and runs on the
remaining test set. Compare results against run_best_full.py (zero-shot
demographic) to measure the few-shot gain.
"""
...
from prompts import AGE_LABELS, GENDER_LABELS, JOINT_FEWSHOT  # noqa: E402
...
FEWSHOT_CSV = "../data/fewshot_examples.csv"
OUTPUT_CSV = "llm_fewshot_results.csv"
CHECKPOINT_CSV = "llm_fewshot_checkpoint.csv"
...
def print_metrics(gold, pred, labels, task):
    ...
    print(f"\n{'='*60}")
    print(f"{task.upper()} RESULTS  (few-shot)")
    print(f"{'='*60}")
    print(
...
def main():
    # Load few-shot examples and build prompt block
    ...
    for i in range(start_idx, len(df)):
        ...
        prompt = JOINT_FEWSHOT.replace("{EXAMPLES}", examples_block).replace(
            "{TEXT}", text
        )
        try:
